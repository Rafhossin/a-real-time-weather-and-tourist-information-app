...
// MARK:   published variables
    ...
    //The getCoordinatesForCity function uses the CLGeocoder to asynchronously fetch geographical coordinates for the given city. It updates the coordinate and region properties on the main thread once the coordinates are obtained.
...
// and specify the map region
        ...
        if let placemarks = try? await geocoder.geocodeAddressString(city),
           let location = placemarks.first?.location?.coordinate {
            ...
            print("Error: Unable to find the coordinates for the location.")
        }
    }
    ...
    func loadData(lat: Double, lon: Double) async throws -> WeatherDataModel {
        ...
        if let url = URL(string: "https://api.openweathermap.org/data/3.0/onecall?lat=\(lat)&lon=\(lon)&units=metric&appid=22621b12febace9d98d7f70397b367b0") {
...
// MARK:  The code below is to help you see number of records and different time stamps that has been retrieved as part of api response.
    ...
    //method attempts to decode a list of locations from a JSON file. If successful, this list is returned; otherwise, the error is printed.
   ...
   //Generates the full URL string for a weather condition icon based on the provided icon code.
    ...
    //method attempts to load annotation data from a JSON file named "places" and store it in the annotations property.
    func fetchAllAnnotations() {
        guard let url = Bundle.main.url(forResource: "places", withExtension: "json"),
              let data = try? Data(contentsOf: url)
        else
        {
            print("Jason file or data not found")
            return
        }
        
        do{
            let decodeData = try JSONDecoder().decode([Location].self, from: data)
            DispatchQueue.main.async{
                self.annotations = decodeData
                
            }
            
        }catch{
            print("Decording failed : \(error)")
        }
    }
